...
use crate::libcpp::LibCppFnMut as Inner;
#[cfg(unix)]
use crate::libstdcpp::LibstdCppFnMut as Inner;
#[cfg(windows)]
use crate::msvc::MsvcCppFnMut as Inner;

#[repr(transparent)]
pub struct CppFnMut<'a, F: 'static + Copy> {
    inner: Inner<'a, F>,
}

impl<'a, R: 'static> CppFnMut<'a, fn() -> R> {
    #[inline]
...
impl<'a, R: 'static, A0: ConvertArg> CppFnMut<'a, fn(A0) -> R> {
    #[inline]
...
impl<'a, R: 'static, A0: ConvertArg, A1: ConvertArg> CppFnMut<'a, fn(A0, A1) -> R> {
    #[inline]
...
mod tests {
    ...
    #[test]
    fn nullary_cxxfn() {
        let mut cxxfn = CppFnMut::<fn() -> u64>::new(66u64, |data| -> u64 {
            let value = *data;
            assert!(value == 66 || value == 15);
            *data = 15;
            value
        });
        assert_eq!(cxxfn.invoke(), 66);
        assert_eq!(cxxfn.invoke(), 15);
    }

    #[test]
    fn nullary_cxxfn_with_capture() {
        let mut val = 0u64;
        let mut cxxfn = CppFnMut::<fn() -> u64>::new(&mut val, |data| -> u64 {
            let v = **data;
            **data = v + 1;
            v
        });
    ...
    #[test]
    fn unary_cxxfn() {
        let mut cxxfn = CppFnMut::<fn(ByVal<u64>) -> u64>::new(66u64, |data, val| -> u64 {
    ...
    fn unary_cxxfn_with_capture() {
        let mut val = 0u64;
        let mut cxxfn = CppFnMut::<fn(ByVal<u64>) -> u64>::new(&mut val, |data, val| -> u64 {
            let v = **data;
            **data = v + val;
            v
        });
    ...
    fn binary_cxxfn() {
        let mut cxxfn =
            CppFnMut::<fn(ByVal<u64>, ByVal<u64>) -> u64>::new((), |_, val, val2| -> u64 {
                val + val2
            });
    ...
    #[test]
    fn trinary_cxxfn() {
        let mut cxxfn = CppFnMut::<fn(ByVal<u64>, ByVal<u64>, ByVal<u64>) -> u64>::new(
            (),
    ...
    #[test]
    fn quaternary_cxxfn() {
        let mut cxxfn = CppFnMut::<fn(ByVal<u64>, ByVal<u64>, ByVal<u64>, ByVal<u64>) -> u64>::new(
            (),
    ...
    fn nullary_heap_cxxfn() {
        let mut cxxfn = CppFnMut::<fn() -> u64>::new((0u64, 0u64), |data| -> u64 {
    ...
    #[test]
    fn unary_heap_cxxfn() {
        let mut cxxfn = CppFnMut::<fn(ByVal<u64>) -> u64>::new((0u64, 0u64), |data, val| -> u64 {
            assert_eq!(*data, (0u64, 0u64));
            assert_eq!(val, 67u64);
            val
        });
    ...
    fn binary_heap_cxxfn() {
        let mut cxxfn = CppFnMut::<fn(ByVal<u64>, ByVal<u64>) -> u64>::new(
            (0u64, 0u64),
            |_, val, val2| -> u64 { val + val2 },
        );
        assert_eq!(cxxfn.invoke(&0, &0), 0);
        assert_eq!(cxxfn.invoke(&66, &0), 66);
        assert_eq!(cxxfn.invoke(&66, &67), 66 + 67);
